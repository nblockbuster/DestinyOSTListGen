...
class Program
{
    ...
    static int Main(string[] args)
    {
        if (args.Length == 0 || args.Contains("--help") || args.Contains("-h") || args.Contains("-?"))
        {
            ...
            Console.WriteLine("Usage: DestinyOSTListGen.exe {path-to-bnk-or-pkgs-folder} [optional: --compare / -c | --sfx / -s]");
        ...
        {
            Parallel.ForEach(Directory.GetFiles(bnk_or_pkgs_dir), bnk =>
            {
